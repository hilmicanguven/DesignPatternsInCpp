...
Bu kalıp metin ("text") tabanlı inputları alıp onları işleyen ve anlamlandırmayı hedefler.
Genellikle text'leri "token" leri ayırır (Lexing)
                (bir cümleyi kelimelere bölmek, bir matematik işlemini sayılar ve operator e göre ayırmak)
        ve bu token'lere göre ifadeyi anlamlı hale getirir (Parsing)

Bir matematik expression'da, ör. (5-4)+(6-5), Token'ler şunlar olacaktır -> (integer sayı, plus/minus sign, left/right parantez)
*/
